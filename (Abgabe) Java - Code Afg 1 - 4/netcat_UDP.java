...
public class netcat_UDP {
    ...
    private static void listenAndTalk ( int port ) throws IOException  {
        ...
        do {
            ...
            int targetPort = p.getPort();
            //<<<<<<
            line = new String(buffer,0,p.getLength(),"UTF-8");
            System.out.println(line);
            //>>>>>Aufgabe 2
            buffer = answer.getBytes("UTF-8");
            DatagramPacket k = new DatagramPacket(buffer,buffer.length,target,targetPort);
            s.send(k);
            //<<<<<<
    ...
    private static void connectAndTalk ( String other_host, int other_port ) throws IOException {
        ...
        do {
            ...
            s.send(p);
            //>>>>>Aufgabe 2
            DatagramPacket k = new DatagramPacket(buffer,buffer.length);
            s.receive(k);
            line = new String(buffer,0,k.getLength(),"UTF-8");
            System.out.println(line);
            //<<<<<<
        } while (!line.equalsIgnoreCase("stop"));
        s.close();
